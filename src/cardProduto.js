...
export function renderizarCatalogo() {
	for (const produtoCatalogo of catalogo) {
		const cartaoProduto = `
        <div class='border-solid border-2 border-slate-300 rounded-xl col-span-1 flex flex-col p-2 justify-between group' id="card-produto-${produtoCatalogo.id}">
        <img
        src="./assets/img/${produtoCatalogo.imagem}"
        alt="Produto ${produtoCatalogo.id} do MercadoJS."
        class="rounded-t-xl shadow-lg z-10 group-hover:scale-[102%] duration-300"
        />
        <div class="rounded-md bg-gradient-to-b from-stone-200 to-slate-300">
